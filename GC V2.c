...
void OpenWhenClosed()
{
  ...
  switch(WorkingMode)
  {
    case 0: //Start stop mode
      ...
      SignalingSystem_AddSignal(&SigSys,2,7); //Stop off
      SignalingSystem_AddSignal(&SigSys,3,4); //Start on
      SignalingSystem_AddSignal(&SigSys,4,5); //Start off
      DoorActFlag=1;
      break;

    case 1: // Open stop close mode
      SignalingSystem_AddSignal(&SigSys,1,6); //Stop on
      SignalingSystem_AddSignal(&SigSys,2,7); //Stop off
      SignalingSystem_AddSignal(&SigSys,3,4); //Start on
      SignalingSystem_AddSignal(&SigSys,4,5); //Start off
